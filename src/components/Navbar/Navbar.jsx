import React, { useState, useEffect } from "react";
import MobileMenu from "../MobileMenu/MobileMenu";
import hamburger from "../../assets/hamburger.png";
import cross from "../../assets/cross.png";
import './Navbar.css'
import NavItemsIn from "./NavItemsIn";
import NavItemsOut from "./NavItemsOut";
import { useNavigate } from "react-router-dom";

const Navbar = (props) => {
  const navigate = useNavigate();
  const [isMobile, setIsMobile] = useState(false);
  const [showModal, setShowModal] = useState(false);

  useEffect(() => {
    const handleResize = () => {
      setIsMobile(window.innerWidth <= 576);
    };
    handleResize();
    window.addEventListener("resize", handleResize);

    return () => {
      window.removeEventListener("resize", handleResize);
    };
  }, []);

  const toggleModal = () => {
    setShowModal(!showModal);
  };

  return (
    <div className='navbar-navbarWrapper'>
      <div
        className= {`navbar-navbarContainer ${showModal ? "navbar-noboxshadow" : "navbar-boxshadow"}`}
       
      >
        <div onClick={() => navigate("/")} className='navbar-logo'>
          SwipTory
        </div>
        <div className='navbar-btnContainer'>
          {!isMobile ? (
            <>
              {props.authValidated ? (
                <NavItemsIn showModal={showModal} />
              ) : (
                <NavItemsOut />
              )}
            </>
          ) : (
            <>
              <img
                onClick={toggleModal}
                className={`navbar-toggleIcon ${
                  showModal ? 'navbar-cross' : 'navbar-hamburger'
                }`}
                src={showModal ? cross : hamburger}
                alt={showModal ? "navbar-cross" : "navbar-hamburger"}
              />
            </>
          )}
        </div>
      </div>

      {showModal && isMobile && (
        <MobileMenu
          authValidated={props.authValidated}
          toggleModal={toggleModal}
        />
      )}
    </div>
  );
};

export default Navbar;