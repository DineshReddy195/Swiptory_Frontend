...
const StoryViewer = (props) => {
    ...
    const fetchBookmarkStatus = async () => {
      try {
        const response = await fetch(
          `https://swiptory-backend-w1mb.onrender.com/api/user/isBookmarked/${slides[currentSlideIndex]._id}`,
          {
            method: "GET",
            headers: {
  ...
  const handleBookmark = async (slideIndex) => {
    try {
      ...
      const endpoint = bookmarkStatus[slideIndex]
        ...
        : "addBookmark";
      const response = await fetch(
        `https://swiptory-backend-w1mb.onrender.com/api/user/${endpoint}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleLike = async (slideIndex) => {
    try {
      const slideId = slides[slideIndex]._id;
      const response = await fetch(
        `https://swiptory-backend-w1mb.onrender.com/api/user/like`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleShare = (slideIndex) => {
    const link = `https://swiptory-backend-w1mb.onrender.com/?slide=true&id=${slides[slideIndex]._id}`;
  ...
  return (
    <div className='overlay'>
      <div className='storyViewerContainer'>
        {!props.isMobile && (
          <img
              ...
              return (
                <div
                  ...
                  className={`progressBar ${
                    isCompleted ? 'progressBarCompleted' : ""
                  } ${isActive ? 'progressBarActive' : ""}`}
                ></div>
              );
            })}
          </div>
